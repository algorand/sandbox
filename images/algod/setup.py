...
def configure_data_dir(network_dir, token, algod_port, kmd_port, bootstrap_url):
    ...
    with open(join(node_dir, 'config.json'), 'w') as f:
        f.write('{ "Version": 12, "GossipFanout": 1, "EndpointAddress": "0.0.0.0:%s", "DNSBootstrapID": "%s", "IncomingConnectionsLimit": 0, "Archival":false, "isIndexerActive":true, "EnableDeveloperAPI":true}' % (algod_port, bootstrap_url))
